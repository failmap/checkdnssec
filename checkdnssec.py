"""Verify if domain name uses DNSSEC."""
...
def get_dnssec(domain_name, dnsresolver=None):
    """Verify if domain name uses DNSSEC.

    :param domain_name: Domain name to verify
    :param dnsresolver: Resolver instance to use (Default value = None)

    """
    # use a default resolver if none provided
    if not dnsresolver:
    ...
    if response.rcode() != 0:
        log.info("get_dnssec_status: rcode was not 0")
        return False
    ...
    if len(answer) != 2:
        ...
        log.info("get_dnssec_status: lenght of answer != 2, " +
                 str(len(answer)))
        return False
    ...
    try:
        ...
        # an exception was raised
        log.info("get_dnssec_status: Failed validation.")
        return False
...
def main():
    """Take domain name from argument and report DNSSEC status."""
    dnsresolver = dns.resolver.Resolver()
    ...
    if get_dnssec(domain_name, dnsresolver=dnsresolver):
